...
namespace moteus
{
        ...
        const auto value = std::min<int32_t>(
            ...
            std::max<int32_t>(
                ...
                static_cast<int32_t>(
                    ...
                    (FastAtan2(
                         static_cast<float>(
                             static_cast<int32_t>(s) - config.common) /
                             2048.0f,
                         static_cast<float>(
                             static_cast<int32_t>(c) - config.common) /
                             2048.0f) +
                     kPi))));
  ...
  private:
    ...
    void ISR_PollI2c()
    {
      ...
      for (size_t i = 0; i < i2c_state_.size(); i++)
      {
        ...
        if (!state.initialized)
        {
          ...
          case DC::kAs5600:
          {
            encoder_raw_data_[0] = 0 | (0 << 5) // watchdog = off
                                   | (1 << 2)   // fast filter threshold = 6/1
                                   | (3 << 0)   // slow filter = 2x
                                   | 0;
            encoder_raw_data_[1] = 0 | (0 << 6) // PWM frequency = 115Hz
    ...
    void HandleConfigUpdate()
    {
      ...
      for (auto &in : digital_inputs_)
      {
        in.reset();
      }
      for (auto &out : digital_outputs_)
      {
        out.reset();
      }
      ...
      // Don't reset RS422 pins here - they will be set correctly
      // when UART is configured based on config_.uart.rs422
      ...
      for (auto &pin : hw_config_.pins)
      {
        ...
        // Don't reset RS422 pins to analog mode
        if (pin.mbed == hw_config_.options.rs422_re ||
            pin.mbed == hw_config_.options.rs422_de)
        {
          continue;
        }
      ...
      if (any_i2c)
      {
        ...
        for (size_t i = 0; i < pin_count_; i++)
        {
          if (config_.pins[i].mode != aux::Pin::Mode::kI2C)
          {
            continue;
          }
      ...
      if (config_.spi.mode != aux::Spi::Config::kDisabled)
      {
        ...
        case aux::Spi::Config::kCuiAmt22:
        {
          ...
          if (options.frequency > 2000000)
          {
            options.frequency = 2000000;
          }
          if (options.frequency < 600000)
          {
            options.frequency = 600000;
          }
        ...
        case aux::Spi::Config::kMa732:
        {
          ...
          options.enable_trim =
              (config_.spi.trim == T::kNone) ? 0 : (config_.spi.trim == T::kTrimX) ? 1
      ...
      if (config_.hall.enabled)
      {
        ...
        for (size_t i = 0; i < pin_count_; i++)
        {
          if (config_.pins[i].mode == aux::Pin::Mode::kHall)
          {
            count++;
          }
        }
        ...
        for (size_t i = 0; i < pin_count_; i++)
        {
          ...
          if (cfg.mode != aux::Pin::Mode::kHall)
          {
            continue;
          }
          const auto mbed = [&]()
      ...
      if (config_.uart.mode != aux::UartEncoder::Config::kDisabled)
      {
        ...
        if (rs422_de_)
        {
          rs422_de_->write(config_.uart.rs422);
        }
        if (rs422_re_)
        {
          rs422_re_->write(!config_.uart.rs422);
        }
      ...
      if (ic_pz_)
      {
        AddSampleType(SampleType::kIcPz, true, true);
      }
      ...
      if (config_.hall.enabled)
      {
        AddSampleType(SampleType::kHall, false, true);
      }
      if (quad_)
      {
        AddSampleType(SampleType::kQuad, false, true);
      }
      if (index_)
      {
        AddSampleType(SampleType::kIndex, false, true);
      }
      if (aksim2_)
      {
        AddSampleType(SampleType::kAksim2, false, true);
      }
      if (cui_amt21_)
      {
        AddSampleType(SampleType::kCuiAmt21, false, true);
      }
      if (i2c_)
      {
        AddSampleType(SampleType::kI2c, false, true);
      }
